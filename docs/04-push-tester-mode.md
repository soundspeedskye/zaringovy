...
- 로그인한 iOS 사용자의 Expo Push Token을 `device_push_tokens`에 저장한다.
- 토큰이 바뀌면 `addPushTokenListener`로 서버에 다시 동기화한다.
- `send-push` Edge Function이 서버 전용 secret으로 인증된 요청만 받고, `audience: "user"`로 한 사용자에게만 발송한다.
- 발송 대상은 `is_enabled = true`인 토큰으로 제한되며, `DeviceNotRegistered` 토큰은 비활성화한다.
...
MVP에서는 `device_id`를 비워 계정의 모든 활성 기기를 대상으로 할 수 있다. 특정 iPhone 한 대만 검사해야 할 때만 `device_id`를 채운다. Expo Push Token 원문을 별도 테이블에 복사하지 않고 기존 `device_push_tokens`를 조회한다.
...
3. `device_push_tokens`에서 해당 `user_id`와 선택적 `device_id`에 매칭되는 활성 Expo 토큰만 조회한다.
...
- 대상 사용자가 탈퇴하거나 토큰이 `DeviceNotRegistered`이면 해당 토큰은 기존 규칙대로 발송 대상에서 제거한다.
...
3. `send_once`로 한 번 보내고, Expo 접수 결과와 기기 수신을 각각 확인한다.
...
5. `send-push`와 공통화할 토큰 조회·Expo 응답 처리 정리
...
기존 토큰 등록 및 앱 수신 코드는 유지한다. 특히 현재 앱은 iOS만 원격 푸시 등록 대상으로 삼고 있으므로, 이 설계를 먼저 적용하면 iOS 테스터를 대상으로 동작한다. Android는 FCM 자격증명과 Android 토큰 등록을 별도 작업으로 추가한 뒤 같은 테스트 대상 모델을 재사용한다.

Expo SDK 57 문서 기준으로 Expo Push Token은 Expo Push Service를 통한 발송에 사용되고, 토큰 변경 시 `addPushTokenListener`로 백엔드 동기화를 처리할 수 있다. Android의 Expo Go에서는 SDK 53부터 원격 푸시를 사용할 수 없으므로 development build가 필요하다.
