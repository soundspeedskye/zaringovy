...
/** FCM은 Android와 iOS 네이티브 빌드에서 모두 사용할 수 있다. */
export function supportsPushNotifications(): boolean {
  return Platform.OS === 'ios' || Platform.OS === 'android';
}

/** 포그라운드 알림도 배너·알림 목록에 표시한다. */
...
/**
 * 권한을 확인하고 FCM 등록 토큰을 현재 로그인 계정에 연결한다.
...
export async function syncPushToken(token: string): Promise<void> {
  ...
  // FCM 토큰은 계정이 아니라 앱 설치/기기 단위다. 계정을 바꾸면 서버 RPC가
...
function hasPushPermission(
  ...
  if (Platform.OS === 'android') return permission.granted;
...
/**
 * IDFV는 iOS에서, Android ID는 Android에서 같은 앱 설치를 구분한다.
 * 기기 식별자를 가져오지 못하면 토큰 등록을 다음 앱 실행으로 미룬다.
 */
async function getPushDeviceId(): Promise<string> {
  if (Platform.OS === 'android') {
    const androidId = Application.getAndroidId();
    if (!androidId) {
      throw new Error('Android 기기 식별자를 아직 가져오지 못해 푸시 토큰 등록을 다시 시도해야 해요.');
    }
